...
namespace aptitude
{
  namespace apt
  {
    ...
    public:
      ...
      {
	return dpkg_status_message(error, std::string(), std::string(), percent, text);
      }
      ...
      static dpkg_status_message make_conffile(const std::string &existing_filename,
					       ...
					       const std::string &text)
      {
	return dpkg_status_message(conffile, existing_filename, new_filename, percent, text);
      }

      static dpkg_status_message make_status(double percent,
					     const std::string &text)
      {
	return dpkg_status_message(status, std::string(), std::string(), percent, text);
      }
      ...
      /** \brief For conffile messages, get the name of the file.
       *  with this
       */
    };

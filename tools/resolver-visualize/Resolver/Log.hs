...
data PartialStep = PartialStep { -- | The search node generated by this step.
                                 ...
                                 pstepReverseSuccessors :: ![(Solution, LinkChoice)],
                                 -- | Promotions generated at
                                 -- this step.
...
successorsStart     = compile "Generating successors for (.*)$"
...
data LogParseState = LogParseState {
      ...
      -- Nothing.
      logParseGeneratingSuccessorsInfo :: !(Maybe Dep),
...
getGeneratingSuccessorsInfo :: LogParse (Maybe Dep)
getGeneratingSuccessorsInfo = get >>= return . logParseGeneratingSuccessorsInfo

-- | Not strict in the dependency itself!
setGeneratingSuccessorsInfo :: Maybe Dep -> LogParse ()
...
processSuccessorsStartLine source matches =
    do d <- parseMatch dep source (matches!1)
       d `seq` setGeneratingSuccessorsInfo $ Just d
       ...
       s `seq`
         modifyLastStep (\lastStep ->
                             let oldSuccessors = pstepReverseSuccessors lastStep
                                 newSuccessors = (s, lastSeenChoice):oldSuccessors in
...
extractPredecessorLinks (step:steps) =
    [(childSolution, (pstepSol step, childChoice))
     | (childSolution, childChoice) <- pstepReverseSuccessors step]
...
extractProcessingSteps partialSteps =
    ...
    -- using those.
    [convert step | step <- partialSteps]
    where
      ...
      findSuccessor :: Solution -> LinkChoice -> Successor
      findSuccessor sol c =
          case Map.lookup sol stepMap of
            Just step -> Successor step c
            Nothing   -> Unprocessed sol c
      ...
      makeStep n pstep =
          let sol        = pstepSol pstep
              successors = reverse $ pstepReverseSuccessors pstep in
          ProcessingStep {
        stepPredecessor = Map.lookup sol parentMap,
        stepSol = sol,
        stepOrder = n,
        stepSuccessors = [findSuccessor sol' c
                              | (sol', c) <- successors],
        stepPromotions = Set.fromList $ pstepPromotions pstep,
        stepTextStart  = pstepTextStart pstep,
        stepTextLength = case pstepTextLength pstep of
                           Just len -> len
...
processFile h =
    do forEachLine h processLogLine
       ...
       let outSteps = extractProcessingSteps $ reverse stepsReversed
